class AssetManager {
    constructor() {
        this.apiUrl = "./apiData.json";
        this.assetsContainer = document.querySelector(".assets");
    }
    ...
    renderData(data) {
        ...
        this.updateTaskInfo(tasks[0]);
        this.renderAssets(tasks[0].assets || []);
    }
    ...
    updateTaskInfo(task) {
        ...
        descriptionElement.textContent = task.task_description;
        jb_title.textContent = task.task_title;
        
        this.updateJourneyBoard(task.assets || []);
    }

    updateJourneyBoard(assets) {
    ...
    renderAssets(assets) {
        assets.forEach(asset => {
            ...
            this.assetsContainer.appendChild(assetElement);
        });
    }

    createAssetElement(asset) {
        const element = document.createElement("div");
        element.className = "asset shadow-gray-300 shadow-md rounded-lg h-[35rem]";
        element.innerHTML = this.getAssetHTML(asset);
        return element;
    }
    ...
    getAssetHTML(asset) {
        return `
            ...
            <div class="description mt-2 p-2">
                <span class="font-semibold">Description:</span> ${asset.asset_description}
            </div>
            <div class="content mt-2">
                ${this.getContentByType(asset)}
            </div>`;
    }

    getContentByType(asset) {
        switch(asset.asset_content_type) {
            case 'video': return this.getVideoContent(asset);
            case 'threadbuilder': return this.getThreadBuilderContent();
            case 'article': return asset.asset_type === 'input_asset' 
                ? this.getArticleInputContent() 
                : this.getArticleDisplayContent();
            default: return '<p>No content available</p>';
        }
    }

    getVideoContent(asset) {
        return `
    ...
    initializeThreadControls() {
        // Implementation of initializeThreadControls method
    }

    initializeNoticeBoard() {
        // Implementation of initializeNoticeBoard method
    }
}
